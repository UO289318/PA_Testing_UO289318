...
public class InscriptionsModel {
     ...
     * Criterios: Actives (o CLOSED pa viajes al pasado), en periodu d'inscripción y con places reales disponibles.
     */
    public List<FormativeActionDTO> getAvailableCourses(Date currentDate){
        ...
        // 🦋 Consulta limpia: Trae activos y CERRADOS (para el Efecto Mariposa)
        ...
        // Buscai los q tien confirmed y reservaos pa comprobar los spots. Ignoramos el futuro.
    ...
    public synchronized void enrollProfessional(ProfessionalDTO profesional, int actionId, Date simulatedDate){
        ...
        // Inscribir en la fecha simulada (y desencadenar efecto mariposa si aplica)
    ...
    // Calculates the future date in 47 working hrs (Estricto).
    ...
    // Same as the previous method but checks the past 47 wrkng hrs.
    ...
    private int getOrCreateProfessional(ProfessionalDTO profesional){
        ...
        if (result.isEmpty()) {
            // 1. No existe ni el email ni el teléfono. Es un profesional totalmente nuevo.
            ...
            return newResult.get(0).getProfessionalId();
                   ...
                   && result.get(0).getEmail().equalsIgnoreCase(profesional.getEmail()) 
                   && result.get(0).getPhone().equals(profesional.getPhone())) {
            
            // 2. Existe exactamente 1 registro, y el email y teléfono coinciden a la perfección.
            return result.get(0).getProfessionalId();
            
        } else {
            // 3. BLOQUEO: O bien el email existe con otro teléfono, o el teléfono con otro email.
    ...
    private void validateAvailableSpots(int actionId, Date simulatedDate){
        ...
        // 🦋 CAMBIO: Permitimos matricular en cursos CLOSED (para reabrirlos si estamos en fecha)
    ...
    private void createInscription(int professionalId, int actionId, Date simulatedDate){
        ...
        // 1. INSERCIÓN BASE: Creamos la nueva matrícula con estado RECEIVED
        ...
        // 🦋 2. EFECTO MARIPOSA (Unhandled Registration / Reactivación)
        // Si alguien viaja al pasado y se inscribe en un curso que estaba CLOSED, alteramos la línea temporal reabriendo el curso.
     ...
     * SINGLE SOURCE OF TRUTH (Auditoría basada en la tabla Payment).
     */
    public void checkAndReleaseExpiredBookings(Date simulatedDate){
        ...
        // 🧹 1. LIMPIEZA INICIAL: Si el profesor lo puso CONFIRMED pero NO hay registro de pago -> RECEIVED
        ...
        // 🕵️‍♂️ 2. AUDITORÍA DE PAGOS
        // Traemos TODAS las inscripciones que SÍ tienen un pago asociado.
        ...
        // ⏳ 3. CANCELACIÓN POR PASO DEL TIEMPO 
