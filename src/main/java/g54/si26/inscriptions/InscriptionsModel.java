...
public class InscriptionsModel {
    	...
    	private String calculateFutureDate(Date simulatedDate){
    		...
    		while (hoursFuture > 0){
    			...
    			if(future.getDayOfWeek() != java.time.DayOfWeek.SATURDAY && future.getDayOfWeek() != java.time.DayOfWeek.SUNDAY)
    				hoursFuture--;
    		}
        ...
        if(Integer.parseInt(enrolledRows.get(0)[0].toString()) >= maxSpots)
        		throw new ApplicationException("Course is full.");
    	}
    	...
    	private void validateNoDuplicateEnrollment(int professionalId, int actionId, Date simulatedDate){
    		...
    		if(!db.executeQueryArray(sql, professionalId, actionId, d).isEmpty())
    			throw new ApplicationException("Already enrolled.");
    	}
        ...
        for(Object[] row : auditRows){
            ...
            if(payDateStr.compareTo(maxValidDateStr) > 0)
                db.executeUpdate("UPDATE Inscription SET state = 'CANCELLED' WHERE inscription_id = ?", id);
            		...
            		if("RECEIVED".equals(row[3].toString())) 
    ...
    private Date timestampToDate(String timestamp){
        try {
        		return new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(timestamp.length() == 10 ? timestamp + " 00:00:00" : timestamp);
        	}
        catch (Exception e){
        		return new Date();
        	}
