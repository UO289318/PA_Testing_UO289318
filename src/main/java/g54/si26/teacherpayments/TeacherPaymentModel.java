 ...
 * Updated to support the new MoneyMovement schema (Payment table removed).
 */
public class TeacherPaymentModel {
    ...
    public List<TeacherInvoiceDTO> getPendingInvoices() {
        ...
        String sql = "SELECT " +
                     ...
                     "(SELECT COALESCE(SUM(ABS(mm.amount)), 0) FROM MoneyMovement mm " +
                     " WHERE mm.invoice_id = i.invoice_id AND mm.type = 'PAYMENT') AS amountPaid " +
                     "FROM Invoice i " +
     ...
     * Updates: inserts a real MoneyMovement (Payment table logic removed).
     */
    public void registerTeacherPayment(int invoiceId, double amount, String transferDateStr) {
        ...
        // 4. Record Money Movement (Real)
        String sqlInsertMovement = "INSERT INTO MoneyMovement (movement_date, amount, status, type, invoice_id) VALUES (?, ?, ?, ?, ?)";
        db.executeUpdate(sqlInsertMovement, transferDateStr, amount, "EXECUTED", "PAYMENT", invoiceId);
